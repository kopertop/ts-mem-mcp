...
export class Database {
	...
	public async addMemory(memory: Memory): Promise<string> {
		...
		const { id, sessionId, agentId, content, metadata, embedding, createdAt, updatedAt } = memory;
		...
		await this.db.run(
			`INSERT INTO memories
				(id, sessionId, agentId, content, metadata, embedding, dimensions, createdAt, updatedAt)
				VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
			[
				id,
				sessionId || null,
